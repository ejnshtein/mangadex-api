...
class Scraper {
  ...
  static parseUser(html) {
    ...
    const user = {
      ...
      language: content('#content > .card > .card-header > span.flag').attr(
        'title'
      ),
      joined: content(
        '#content > .card > .card-body > .row > div:last-of-type > div:nth-child(2) > div:last-of-type'
      )
        .text()
        .trim(),
      photo_url: content(
        '#content > .card > .card-body > .row > div:first-of-type > img'
      ).attr('src')
    }
        ...
        switch (true) {
          case /stats/i.test(title):
            return {
              ...
              stats: {
                views: parseInt(
                  selector('div:last-of-type > ul > li:first-of-type')
                    .text()
                    .trim()
                    .replace(/,/i, '')
                ),
                uploads: parseInt(
                  selector('div:last-of-type > ul > li:last-of-type')
                    .text()
                    .trim()
                    .replace(/,/i, '')
                )
          ...
          case /last online/i.test(title):
            return {
              _: 'last_online',
              last_online: selector('div.col-lg-9')
                .text()
                .trim()
                .replace(/\\n\s+/i, '')
            }
  ...
  static parseLogin(html) {
    ...
    const isLogined = /you are logged in/i.test(
  ...
  static parseGroup(groupId, html) {
    ...
    const group = {
      id: parseInt(groupId),
      name: content('#content > div.card > .card-header > span.mx-1')
        .text()
        .trim(),
      ...
      stats: {
        views: parseInt(
          content(
            '#content > .row > .col-md-6:first-of-type > .card > .table > tbody > tr:nth-child(2) > td > ul > li:first-of-type'
          )
            .text()
            .trim()
            .replace(/,/i, '')
        ...
        follows: parseInt(
          content(
            '#content > .row > .col-md-6:first-of-type > .card > .table > tbody > tr:nth-child(2) > td > ul > li:nth-child(2)'
          )
            .text()
            .trim()
            .replace(/,/i, '')
        ),
        total_chapters: parseInt(
          content(
            '#content > .row > .col-md-6:first-of-type > .card > .table > tbody > tr:nth-child(2) > td > ul > li:nth-child(3)'
          )
            .text()
            .trim()
            .replace(/,/i, '')
      ...
      links: content(
        '#content > .row > .col-md-6:first-of-type > .card > .table > tbody > tr:nth-child(3) > td'
      )
        .children('a')
        .map((id, el) => {
          const selector = cheerio.load(el)
      ...
      leader: {
        id: parseInt(
          content(
            '#content > .row > .col-md-6:nth-child(2) > .card > .table > tbody > tr > td > a'
          ).attr('id')
        ),
        username: content(
          '#content > .row > .col-md-6:nth-child(2) > .card > .table > tbody > tr > td > a'
      ...
      members: content(
        '#content > .row > .col-md-6:nth-child(2) > .card > .table > tbody > tr:nth-child(2) > td > ul > li'
      )
        .map((id, el) => {
          const selector = cheerio.load(el)
        ...
        .get(),
      upload_restrictions: content(
        '#content > .row > .col-md-6:nth-child(2) > .card > .table > tbody > tr:nth-child(3) > td > span'
      ).attr('title'),
      description: content('#content > .card:nth-child(4) > .card-body')
        .html()
        .trim()
    }
  ...
  static parseHome(html, host) {
    ...
    const home = {
      ...
      top_chapters: {
        six_hours: content('#six_hours > ul > li').map(parseTopChapters).get(),
        day: content('#day > ul > li').map(parseTopChapters).get(),
        week: content('#week > ul > li').map(parseTopChapters).get()
      },
      top_manga: {
  ...
  static parseMe(html) {
    ...
    const id = parseInt(
      content(
        '#navbarSupportedContent > ul.navbar-nav:last-of-type > li:nth-child(2) > a'
      )
        .attr('href')
        .split('/')[2]
    )
