import Joi from 'joi'
import { MappingIdAttributes } from '../../types/data-types/legacy'
import { apiBase, apiResponse } from './base'

export const legacyTypeSchema = Joi.string().valid(
  'group',
  'manga',
  'chapter',
  'tag'
...
export const mappingIdAttribute = Joi.object<MappingIdAttributes>({
  type: legacyTypeSchema.required(),
  legacyId: Joi.number().required(),
  newId: Joi.string().required()
})

export const mappingIdSchema = apiBase(
  'mapping_id',
  mappingIdAttribute.required()
)

export const mappingIdResponseSchema = Joi.array().items(
  apiResponse({
