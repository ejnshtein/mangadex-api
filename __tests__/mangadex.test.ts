...
import { mappingIdResponseSchema } from '../jest/schema/mangadex'
...
describe('mangadex api', () => {
  ...
  it('should convert legacy chapter id to new id', async () => {
    const client = await getClient()

    const result = await client.convertLegacyId([300859], 'chapter')

    const validateResult = mappingIdResponseSchema.validate(result)
